...
void (*SdFile::dateTime_)(uint16_t* date, uint16_t* time) = 0;

#if ALLOW_DEPRECATED_FUNCTIONS
// suppress cpplint warnings with NOLINT comment
void (*SdFile::oldDateTime_)(uint16_t& date, uint16_t& time) = 0;  // NOLINT
...
// return pointer to cached entry or 0 for failure
...
uint8_t SdFile::contiguousRange(uint32_t* bgnBlock, uint32_t* endBlock) {
	...
	for (uint32_t c = firstCluster_;; c++) {
		...
		if (next != (c + 1)) {
			...
			*endBlock = vol_->clusterStartBlock(c) + vol_->blocksPerCluster_
					- 1;
			return true;
		}
	}
 ...
 *
 */
uint8_t SdFile::createContiguous(SdFile* dirFile, const char* fileName,
		uint32_t size) {
	...
	while (nToWrite > 0) {
		...
		if (n == 512) {
			...
			if (blockOffset == 0 && curPosition_ >= fileSize_) {
				...
				if (!SdVolume::cacheRawBlock(block,
						SdVolume::CACHE_FOR_WRITE)) {
					goto writeErrorReturn;
				}
			}
