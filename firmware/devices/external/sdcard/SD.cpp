...
bool walkPath(char *filepath, SdFile& parentDir,
		bool (*callback)(SdFile& parentDir, char *filePathComponent,
				bool isLastComponent, void *object), void *object = 0) {
	/*
	...
	while (true) {
		...
		bool shouldContinue = callback((*p_parent), buffer, !moreComponents,
				object);
...
int8_t SDClass::begin(DigitalPin *cs, SPI *spi) {
	/*
	 ...
	 */
/*	return card.init(cs, spi) &&
	 volume.init(&card) &&
	 root.openRoot(volume);
*/
	if (!card.init(cs, spi)) return 1;
